...
1. [dev.mysql.com/downloads/mysql](https://dev.mysql.com/downloads/mysql) 접속
2. **Windows (x86, 64-bit), MSI Installer** 다운로드
3. 설치 진행 시 아래 설정으로 맞추세요:
   - Setup Type: **Developer Default** 선택
   - Root 비밀번호: `123456` 입력 (팀 공통 설정)
4. 설치 완료 후 MySQL이 자동으로 실행됩니다.

설치 확인 (CMD 또는 PowerShell에서):
```bash
mysql -u root -p
```
비밀번호 `123456` 입력 후 `mysql>` 프롬프트가 뜨면 성공입니다.

**DB 초기화 (최초 1회)**

`database/init.sql` 파일을 실행해 DB와 테이블을 생성합니다:
```bash
mysql -u root -p < database/init.sql
```

> **MySQL**이란? 데이터(회원 정보 등)를 저장하고 관리하는 데이터베이스 프로그램입니다.
...
설정하다가 막히면 → **[TROUBLESHOOTING.md](TROUBLESHOOTING.md)** 먼저 확인해보세요!

실제로 팀원이 막혔던 7가지 문제와 해결 방법이 정리되어 있어요.
